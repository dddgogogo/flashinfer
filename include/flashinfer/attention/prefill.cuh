...
#pragma unroll
        ...
        if (MASK_MODE == MaskMode::kCustom) {
          // Ling's DDTree kCustom verify masks expose the full committed prefix
          // `[0, kv_len - qo_len)` to every query row, so the packed mask is all-ones
          // on the first `prefix_len / CTA_TILE_KV` full tiles -> skip those.
...
__device__ __forceinline__ void BatchPrefillWithPagedKVCacheTQ4Device(
  ...
  // All producer threads dequantize through centroids_smem below.
  block.sync();
...
#pragma unroll 1
  for (uint32_t iter = 0; iter < num_iterations; ++iter) {
    ...
    compute_qk<KTraits>(&qo_smem, &q_smem_offset_r, &k_smem, &k_smem_offset_r,
                        /*k_sf_smem=*/nullptr, lane_idx, s_frag);
    uint32_t kv_idx_base =
        chunk_start + (iter * NUM_WARPS_KV + get_warp_idx_kv<KTraits>(tid.z)) * NUM_MMA_KV * 16;
    ...
    // compute softmax * V
    compute_sfm_v<KTraits>(&v_smem, &v_smem_offset_r, /*v_sf_smem=*/nullptr, lane_idx,
                           s_frag, o_frag, d);
